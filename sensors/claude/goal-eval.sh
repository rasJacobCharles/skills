...
#
# Saves a markdown report to ~/.claude/session-evals/
...
command -v claude >/dev/null 2>&1 || { echo "goal-eval: claude not found" >&2; exit 0; }
...
# First 8 non-meta user messages, collapsed to one line each
USER_MSGS=$(jq -r '
...
# Last 6 assistant text blocks, collapsed to one line each.
# Handles both array content (text blocks) and plain string content.
ASSISTANT_MSGS=$(jq -r '
...
# before the background subshell can read it.
PROMPT_FILE=$(mktemp /tmp/goal-eval-XXXXXX.txt)

{
  printf 'You are a session quality evaluator for Claude Code. Analyse this session and complete three tasks.\n\n'
...
# --- Run in background so SessionEnd is not delayed ---
