#!/usr/bin/env bash
# Sensor: skill-usage-log (Antigravity version)
# Triggered on: PostToolUse (matcher: Skill)
#
# Appends one line per skill invocation to $BASE_DIR/skill-usage.log so you can
...
# Extract camelCase fields passed by Antigravity CLI
SKILL_NAME=$(echo "$INPUT" | jq -r '.toolInput.skill // "unknown"' 2>/dev/null || echo "unknown")
ARGS=$(echo "$INPUT"       | jq -r '.toolInput.args  // "(none)"'  2>/dev/null || echo "(none)")
