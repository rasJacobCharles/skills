...
description: Trigger this skill to run code reviews, check security standards, enforce clean code practices, run multi-agent refactoring, or optimize codebase files using continuous feedback loops.
---

# Multi-Agent Code Review Skill

This skill teaches the agent how to coordinate a multi-agent system of seven specialized agents to review, optimize, and validate code changes through a continuous optimization loop with local sandboxing.

---

## 🏛️ Context Engineering & Progressive Disclosure

To keep execution contexts clean and efficient, the instructions and boundaries are distributed across specialized sub-files. Read these files on-demand during execution:

### 1. System Documentation & Rules
*   **[Rules.md](docs/Rules.md)**: Core rules governing the entire system.
*   **[Agent.md](docs/Agent.md)**: The base specification for agent behaviors and personas.
...
*   **[3-structure.md](agents/3-structure.md)**: Structure Agent - architecture and organization.
...
2. **Scoring & Constraints**:
   *   **Passing Score**: The deduction score must be optimized to keep the final passing score above **80%**.
...
2. Prompt the user to either restart the loop or end the loop and finalize changes.
